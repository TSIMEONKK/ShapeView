...
public class ShapeDrawable extends Drawable {
    ...
    public ShapeDrawable setSolidGradientOrientation(ShapeGradientOrientation orientation) {
        // 动态传入 null 时沿用旧默认方向，避免绘制阶段 switch 抛空指针。
        mShapeState.solidGradientOrientation = orientation == null ?
                ShapeGradientOrientation.TOP_TO_BOTTOM : orientation;
        mRectDirty = true;
        invalidateSelf();
        return this;
    }
    ...
    public ShapeDrawable setStrokeColor(@ColorInt int... colors) {
        ...
        // 颜色数量变化后，旧停靠点可能不再匹配；不匹配时回退均分停靠点。
        if (!isGradientPositionsValid(mShapeState.strokeColors, mShapeState.strokePositions)) {
            mShapeState.strokePositions = null;
        }
        if (colors == null || colors.length == 0) {
            // 空数组和 null 都表示清除描边，避免残留旧 Shader 或颜色。
    ...
    public ShapeDrawable setStrokeGradientOrientation(ShapeGradientOrientation orientation) {
        // 动态传入 null 时沿用旧默认方向，避免绘制阶段 switch 抛空指针。
        mShapeState.strokeGradientOrientation = orientation == null ?
                ShapeGradientOrientation.TOP_TO_BOTTOM : orientation;
    ...
    /**
     * 设置描边渐变停靠位置。数组长度必须和描边颜色数一致。
     */
    public ShapeDrawable setStrokeGradientPositions(float[] positions) {
        // 颜色可能稍后才设置，先保留格式合法的位置；颜色设置时会再次校验长度。
        // 配置不合法时使用系统默认均分停靠点，不向业务方抛出异常。
        mShapeState.strokePositions = isGradientPositionsRangeValid(positions) ?
                positions.clone() : null;
        mRectDirty = true;
        invalidateSelf();
        return this;
    }
    ...
    public ShapeDrawable setStrokeGradientAngle(float angle) {
        ...
        mShapeState.strokeGradientAngle = angle;
        mRectDirty = true;
        invalidateSelf();
        return this;
    }
     ...
     */
    public ShapeDrawable setShadowInsetSize(int size) {
        mShapeState.setShadowInsetSize(size);
        mPathDirty = true;
        mRectDirty = true;
        invalidateSelf();
        return this;
    ...
    private boolean ensureValidRect() {
        ...
        if (mRect.isEmpty()) {
            // 空尺寸控件不创建起止点重合的 LinearGradient，避免系统抛参数异常。
        ...
        if (st.strokeColors != null) {
            ...
            float[] coordinate = Float.isNaN(st.strokeGradientAngle) ?
                    ShapeDrawableUtils.computeLinearGradientCoordinate(
                            mLayoutDirection, mRect, level, st.strokeGradientOrientation) :
                    ShapeDrawableUtils.computeLinearGradientCoordinate(mRect, st.strokeGradientAngle);
            try {
                mStrokePaint.setShader(new LinearGradient(coordinate[0], coordinate[1], coordinate[2], coordinate[3],
                        st.strokeColors, st.strokePositions, Shader.TileMode.CLAMP));
            } catch (RuntimeException exception) {
                // 系统对异常颜色/停靠点做最后兜底：先尝试均分停靠点，仍失败则退回首个颜色。
                try {
                    mStrokePaint.setShader(new LinearGradient(coordinate[0], coordinate[1], coordinate[2], coordinate[3],
                            st.strokeColors, null, Shader.TileMode.CLAMP));
                } catch (RuntimeException ignored) {
                    mStrokePaint.setShader(null);
                    mStrokePaint.setColor(st.strokeColors.length > 0 ? st.strokeColors[0] : Color.TRANSPARENT);
                }
            }

            if (!st.hasStrokeColor && mStrokePaint.getShader() != null) {
    ...
    /** 校验 Android LinearGradient 所要求的停靠位置。 */
    private static boolean isGradientPositionsValid(int[] colors, float[] positions) {
        if (colors == null || colors.length < 2 || positions == null || colors.length != positions.length) {
            return false;
        }
        return isGradientPositionsRangeValid(positions);
    }

    /** 校验停靠位置本身，允许在颜色尚未设置时暂存。 */
    private static boolean isGradientPositionsRangeValid(float[] positions) {
