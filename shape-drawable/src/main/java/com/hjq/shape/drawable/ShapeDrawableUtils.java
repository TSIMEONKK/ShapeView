...
final class ShapeDrawableUtils {
   ...
   /**
    * 根据任意角度计算覆盖整个矩形的线性渐变端点。
    * 角度基于 Android 屏幕坐标系：0 度从左至右，90 度从上至下。
    */
   static float[] computeLinearGradientCoordinate(RectF rect, float angle) {
      double radians = Math.toRadians(angle % 360f);
      ...
      // 沿渐变方向投影四个顶点，确保色带完整覆盖控件边界。
      float extent = Math.abs(directionX) * halfWidth + Math.abs(directionY) * halfHeight;
      return new float[] {
              centerX - directionX * extent, centerY - directionY * extent,
              centerX + directionX * extent, centerY + directionY * extent};
   }
